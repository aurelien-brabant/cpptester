...
Tester::Tester(const std::string& progName)
  : _progName(progName)
{
...
Tester::runAllSuites(void)
{
    ...
    std::ofstream timeDumpOfs(
      (std::string("./.castorno/") + _progName + ".time.txt").c_str());
    ...
    for (TestSuiteMap::const_iterator cit = _testSuites.begin();
         cit != _testSuites.end();
         ++cit) {
        cout << "\033[1;0m>>>>>>>>>> \033[0;36m" << cit->first
             << " \033[1;0m<<<<<<<<<<\033[0m\n\n";

        /* For each test */
        size_t passedN = 0;
        for (std::vector<Test>::const_iterator vcit = cit->second.begin();
             vcit != cit->second.end();
             ++vcit) {
            ...
            cout << "[" << (ret ? "\033[0;31mX" : "\033[0;32m✔") << "\033[0m] "
                 ...
                 << (string("\033[0;34m") + vcit->name + "\033[0m");
