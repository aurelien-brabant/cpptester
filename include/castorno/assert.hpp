...
#define assert_throw(ExceptionType, expr)                                      \
    try {                                                                      \
        expr;                                                                  \
        tester.error << "Expected expression to throw a " #ExceptionType       \
                        " but didn't";                                         \
        return 1;                                                              \
    } catch (ExceptionType & e) {                                              \
    } catch (std::exception & e) {                                             \
        tester.error << "Caught exception, but not of type " #ExceptionType    \
                        "!";                                                   \
        return 1;                                                              \
    }
...
bool
range_eq(Tester& tester,
         Iterator actual_begin,
         Iterator actual_end,
         Iterator expected_begin,
         Iterator expected_end)
{
    ...
    while (actual_begin != actual_end) {
        if (*actual_begin != *expected_begin) {
            tester.error << "Ranges differ at element "
...
#define assert_range_eq(                                                       \
  actual_begin, actual_end, expected_begin, expected_end)                      \
    if (!range_eq(                                                             \
          tester, actual_begin, actual_end, expected_begin, expected_end)) {   \
        return 1;                                                              \
    }

#define assert_range_uneq(                                                     \
  actual_begin, actual_end, expected_begin, expected_end)                      \
    if (range_eq(                                                              \
          tester, actual_begin, actual_end, expected_begin, expected_end)) {   \
